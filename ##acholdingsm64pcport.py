...
# =======================================================================
# ULTRAMARIO 4K / AC'S SM64 PC PORT — PYTHON EDITION (hub + courses aligned with sm64ex flow)
# (C) 2026 A.C Holdings / Team Flames
# Full SM64 structure: castle grounds, 3 interior floors, basement, all 15 courses, secrets, Bowser
# Controls match sm64ex-style keyboard defaults:
#   WASD = analog stick | IJKL = C-up/C-left/C-down/C-right (Lakitu cam)
#   Space = A (jump) | Shift = run; B key = B (dive in air)
#   Z / Left Ctrl = Z (crouch ground, ground pound air)
#   Long jump = hold Z or Ctrl + Space while running
#   E or F = enter door / painting | R = camera mode | TAB or M = mouse look
# =======================================================================
...
CAM_DIST = 550
CAM_HEIGHT = 180
CAM_LERP = 0.12              # Snappier camera (sm64ex default)
CAM_ANGLE_SPEED = 0.055      # Q/E rotation speed (matches C-button feel)
CAM_PITCH_SPEED = 0.03
CAM_PITCH_MIN = -0.7
CAM_PITCH_MAX = 0.85
CAM_ZOOM_MIN = 200
CAM_ZOOM_MAX = 1200
CAM_ZOOM_STEP = 40
LAKITU_YAW_CATCHUP = 0.035   # Auto-follow behind Mario
MOUSE_SENS_X = 0.003         # Mouse camera (sm64ex mouselook)
MOUSE_SENS_Y = 0.002
# C-button camera rates (same as prior Q/E + arrows, rebinding only)

# Movement (SM64-accurate analog stick simulation)
MOVE_ACCEL = 2.0              # Snappier acceleration (sm64 feels instant)
MOVE_DECEL = 0.88             # Less ice-skating, more grip
MAX_WALK_SPEED = 8
MAX_RUN_SPEED = 16            # SM64 Mario tops out ~48 units/frame at 30fps
SPRINT_SPEED = 22
JUMP_FORCE = 17               # SM64 initial jump velocity
DOUBLE_JUMP_FORCE = 24
TRIPLE_JUMP_FORCE = 32
LONG_JUMP_FORCE_Y = 12
LONG_JUMP_FORCE_XZ = 24
BACKFLIP_FORCE = 28
SIDE_FLIP_FORCE = 26
WALL_KICK_FORCE = 20
...
DIVE_FORCE_Y = 6
DIVE_FORCE_XZ = 20
GRAVITY = 0.95                # SM64 gravity = ~4 units/frame^2
TERMINAL_VELOCITY = -65
FALL_DAMAGE_THRESHOLD = -42
STAR_TOTAL = 120
TURN_RATE_SLOW = 0.35         # Face-yaw lerp when walking (fast snap)
TURN_RATE_FAST = 0.18         # Face-yaw lerp when running (wider arc)
...
class SM64Audio:
    ...
    def _generate_sounds(self):
        ...
        try:
            ...
            pass
        # Locked door / not enough stars (low buzz)
        s = self._make_sound(120, 0.12, 'square', 0.22)
        if s:
            self.sounds['door_locked'] = s
...
def build_castle_interior_f1():
    ...
    # Basement double doors (SM64 wooden door by Bob-omb portrait wing)
    for dx in (-56, 56):
        m.add_cube(48, 140, 24, 600 + dx, 70, 600, (72, 48, 24))
        m.add_cube(40, 128, 8, 600 + dx, 70, 612, (40, 28, 16))
    m.add_cube(24, 24, 8, 600, 150, 608, (200, 180, 40))
    # Main entrance / exit to castle grounds (back of lobby)
    for dx in (-64, 64):
        m.add_cube(56, 160, 20, dx, 80, 920, (72, 48, 24))
        m.add_cube(48, 148, 6, dx, 80, 932, (40, 28, 16))
    m.add_cube(140, 28, 12, 0, 150, 926, (200, 180, 40))
    # Stair door to second floor (north)
    for dx in (-64, 64):
        m.add_cube(56, 150, 20, dx, 75, -920, (88, 56, 40))
        m.add_cube(48, 138, 6, dx, 75, -932, (56, 36, 20))
    m.add_cube(160, 22, 10, 0, 148, -926, BUTTON_GOLD)
    m.add_cube(100,100,5, 0,320,-998, (248,180,200))
    # Princess's Secret Slide pillar (star door alcove)
    m.add_cube(36, 200, 36, 0, 100, 0, (140, 130, 90))
    m.add_cube(28, 24, 28, 0, 210, 0, RED)
...
def build_castle_upper():
    ...
    # Door to third floor (50-star staircase) and return to main floor
    for dx in (-56, 56):
        m.add_cube(48, 130, 18, dx, 65, -720, (88, 56, 40))
    m.add_cube(130, 22, 8, 0, 132, -728, BUTTON_GOLD)
    for dx in (-56, 56):
        m.add_cube(48, 130, 18, dx, 65, 720, (72, 48, 24))
    return m, [], []
...
def build_castle_top():
    ...
    # Wing Mario Over the Rainbow (hidden portrait wall — sm64 third floor)
    m.add_cube(72, 72, 10, -320, 110, -320, (160, 120, 40))
    for i, col in enumerate([(248, 56, 56), (248, 200, 40), (48, 200, 72), (56, 120, 248)]):
        m.add_cube(16, 56, 4, -320 + (i - 1.5) * 18, 110, -327, col)
    # Return door toward second floor
    for dx in (-48, 48):
        m.add_cube(44, 120, 16, dx, 60, 420, (72, 48, 24))
    m.add_cube(120, 20, 8, 0, 120, 428, BUTTON_GOLD)
    return m, [], []
...
LEVELS = {
    ...
    "castle_f1": {"name":"Inside the Castle (Main Floor)","builder":build_castle_interior_f1,"req":0},
    "castle_basement": {"name":"The Castle Basement","builder":build_castle_basement,"req":0},
    "castle_upper": {"name":"Second Floor","builder":build_castle_upper,"req":3},
    "castle_top": {"name":"Third Floor","builder":build_castle_top,"req":50},
...
# Mario spawn (x,y,z) when entering `to_level` from `from_level` (SM64-style exits)
DEFAULT_MARIO_SPAWN = (0, 50, 400)
ENTRANCE_SPAWNS = {
    ("castle_grounds", "castle_f1"): (0, 50, 920),
    ("castle_f1", "castle_grounds"): (0, 50, -920),
    ("castle_f1", "castle_basement"): (0, 50, 500),
    ("castle_basement", "castle_f1"): (520, 50, 520),
    ("castle_f1", "castle_upper"): (0, 50, -720),
    ("castle_upper", "castle_f1"): (0, 50, 720),
    ("castle_upper", "castle_top"): (0, 50, -520),
    ("castle_top", "castle_upper"): (0, 50, 480),
    ("castle_f1", "s_slide"): (0, 50, 120),
    ("s_slide", "castle_f1"): (0, 50, 120),
}


def get_hub_warp_target(level_id, mario):
    """Return level id to warp to when pressing E/F in a hub, or None."""
    if mario is None:
        return None
    mx, mz, my = mario.x, mario.z, mario.y

    def near(cx, cz, r=150):
        return abs(mx - cx) < r and abs(mz - cz) < r

    if level_id == "castle_grounds":
        if abs(mx) < 120 and mz < -900:
            return "castle_f1"
    elif level_id == "castle_f1":
        for p in CASTLE_F1_PAINTINGS:
            if near(p["pos"][0], p["pos"][2]):
                return p["level"]
        if near(600, 600, 100):
            return "castle_basement"
        if abs(mx) < 200 and mz < -800:
            return "castle_upper"
        if abs(mx) < 200 and mz > 800:
            return "castle_grounds"
        if abs(mx) < 120 and abs(mz) < 120 and my > 50:
            return "s_slide"
    elif level_id == "castle_basement":
        for p in BASEMENT_PAINTINGS:
            if near(p["pos"][0], p["pos"][2]):
                return p["level"]
        if near(800, 800, 100):
            return "s_metal"
        if near(600, -1178, 100):
            return "s_vanish"
        if abs(mx) < 200 and mz > 1000:
            return "castle_f1"
        if near(0, -600, 120):
            return "b1_dark"
    elif level_id == "castle_upper":
        for p in UPPER_PAINTINGS:
            if near(p["pos"][0], p["pos"][2]):
                return p["level"]
        if abs(mx) < 200 and mz < -600:
            return "castle_top"
        if abs(mx) < 200 and mz > 600:
            return "castle_f1"
        if near(600, 0, 200):
            return "b2_fire"
    elif level_id == "castle_top":
        if near(0, 0, 100):
            return "s_tower"
        if near(-320, -320, 130):
            return "s_wing"
        if abs(mx) < 200 and mz > 300:
            return "castle_upper"
        if my > 100 and abs(mx) < 180 and mz < -200:
            return "b3_sky"
    return None


HUB_LEVEL_IDS = frozenset({
    "castle_grounds", "castle_f1", "castle_basement", "castle_upper", "castle_top",
})
...
# =====================================================================
# MAIN LOOP — ULTRAMARIO 4K / SM64 PC PORT (Py)
# =====================================================================
def main():
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption("Ultramario 4K — SM64 PC Port (Py)  |  (C) 2026 A.C Holdings")
    ...
    mouse_cam = False   # SM64 PC port mouse camera (TAB toggle)
    ...
    painting_warp_from = None  # hub/course id before warp (for entrance spawn)
    ...
    def try_warp(target_level):
        """Enter level if star count meets LEVELS[target]['req'] (SM64 key doors)."""
        nonlocal painting_warp_from
        if not target_level or target_level not in LEVELS:
            return
        req = LEVELS[target_level]["req"]
        if len(collected_stars) < req:
            need = req - len(collected_stars)
            show_action(f"Need {req} Stars! ({need} more)")
            audio.play('door_locked')
            return
        painting_warp_from = current_level_id
        start_painting_warp(target_level)

    def load_level(level_id, entered_from=None):
        ...
        if isinstance(result, tuple):
            ...
            current_level_stars = []
            current_level_coins = []
        spawn = DEFAULT_MARIO_SPAWN
        if entered_from is not None:
            spawn = ENTRANCE_SPAWNS.get((entered_from, level_id), DEFAULT_MARIO_SPAWN)
    ...
    def start_painting_warp(target_level):
        nonlocal current_state, painting_warp_timer, painting_warp_target, mouse_captured
    ...
    def draw_sm64_hud():
        ...
        cam_label = "LAKITU" if cam_mode == 0 else "MARIO"
        if mouse_cam: cam_label += " +MOUSE"
        cam_text = font_small.render(cam_label, True, (180, 180, 180))
        screen.blit(cam_text, (WIDTH - 120, HEIGHT - 18))
    ...
    def draw_painting_warp():
        ...
        if painting_warp_timer > 0:
            ...
            if painting_warp_timer <= 0:
                src = painting_warp_from
                painting_warp_from = None
                load_level(painting_warp_target, src)
                current_state = STATE_GAME
                # Re-grab mouse if mouselook was active
                if mouse_cam:
    ...
    while running:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if mario.is_on_ground() and mario.action not in (ACT_GROUND_POUND_LAND, ACT_DIVE_SLIDE, ACT_STAR_DANCE):
                            ...
                            z_held = keys[pygame.K_z] or keys[pygame.K_LCTRL]
                            if z_held and mario.speed > 6:
                                ...
                                if mario.action == ACT_TRIPLE_JUMP:
                                    ...
                                    show_action("JUMP")
                    elif event.key in (pygame.K_z, pygame.K_LCTRL) and mario:
                        if mario.is_airborne():
                            mario.do_ground_pound(audio)
                            show_action("GROUND POUND!")
                        elif mario.is_on_ground():
                            if mario.action != ACT_CROUCH:
                                mario.action = ACT_CROUCH
                                show_action("CROUCH")
                    elif event.key in (pygame.K_b, pygame.K_x) and mario and mario.is_airborne():
                        mario.do_dive(audio)
                        show_action("DIVE!")
                        ...
                        # Toggle mouse camera (sm64ex: TAB / many configs use M)
                        ...
                        if mouse_cam:
                            ...
                            mouse_captured = True
                            pygame.mouse.set_pos(cx, cy)
                            show_action("MOUSE CAM ON")
                        else:
                            pygame.mouse.set_visible(True)
                            pygame.event.set_grab(False)
                            mouse_captured = False
                            show_action("MOUSE CAM OFF")
                    elif event.key in (pygame.K_e, pygame.K_f):
                        # Doors & paintings (star requirements like SM64); courses exit to lobby
                        if current_level_id in HUB_LEVEL_IDS:
                            tgt = get_hub_warp_target(current_level_id, mario)
                            if tgt:
                                try_warp(tgt)
                        elif current_level_id:
                            try_warp("castle_f1")
                elif event.type == pygame.KEYUP:
                    if event.key in (pygame.K_z, pygame.K_LCTRL) and mario:
                        if mario.action == ACT_CROUCH:
        ...
        if current_state == STATE_GAME and mario:
            ...
            # === SM64 PC PORT CAMERA (sm64ex C-buttons on IJKL) ===
            # J / L = C-left / C-right (Lakitu orbit)
            if keys[pygame.K_j]:
                cam_yaw -= CAM_ANGLE_SPEED
            if keys[pygame.K_l]:
                cam_yaw += CAM_ANGLE_SPEED
            # I / K = C-up / C-down (pitch)
            if keys[pygame.K_i]:
                cam_pitch = min(CAM_PITCH_MAX, cam_pitch + CAM_PITCH_SPEED)
            if keys[pygame.K_k]:
                cam_pitch = max(CAM_PITCH_MIN, cam_pitch - CAM_PITCH_SPEED)
            # Mouse camera (sm64ex mouselook — TAB toggle)
            if mouse_cam and mouse_captured:
                mx, my = pygame.mouse.get_pos()
                mdx = mx - cx
                mdy = my - cy
                if abs(mdx) > 0 or abs(mdy) > 0:
                    cam_yaw += mdx * MOUSE_SENS_X
                    cam_pitch -= mdy * MOUSE_SENS_Y
                    cam_pitch = max(CAM_PITCH_MIN, min(CAM_PITCH_MAX, cam_pitch))
                    pygame.mouse.set_pos(cx, cy)
            # Zoom (+/- fine control, R toggle in events)
            if keys[pygame.K_EQUALS] or keys[pygame.K_PLUS]:
                cam_dist = max(CAM_ZOOM_MIN, cam_dist - CAM_ZOOM_STEP * 0.5)
            if keys[pygame.K_MINUS]:
                cam_dist = min(CAM_ZOOM_MAX, cam_dist + CAM_ZOOM_STEP * 0.5)

            # === SM64-STYLE MOVEMENT (WASD + Arrow fallback) ===
            if mario.action not in (ACT_STAR_DANCE, ACT_DEATH, ACT_GROUND_POUND_LAND, ACT_DIVE_SLIDE):
                ...
                # Analog stick = WASD only (sm64ex; arrows reserved for menus / not stick)
                go_fwd = keys[pygame.K_w]
                go_back = keys[pygame.K_s]
                ...
                if go_back:
                    ...
                    moving = True
                if go_left:
                    accel_x -= right_x * MOVE_ACCEL * 0.8 * speed_mult
                    accel_z -= right_z * MOVE_ACCEL * 0.8 * speed_mult
                    moving = True
                if go_right:
                    accel_x += right_x * MOVE_ACCEL * 0.8 * speed_mult
                    accel_z += right_z * MOVE_ACCEL * 0.8 * speed_mult
                ...
                # Face direction of movement (SM64 speed-dependent turn rate)
                if moving and speed > 0.8:
                    target_yaw = math.atan2(-vel_x, -vel_z)
                    diff = target_yaw - mario.face_yaw
                    while diff > math.pi: diff -= 2 * math.pi
                    while diff < -math.pi: diff += 2 * math.pi
                    # SM64: fast snap at walk speed, wide arc at run speed
                    turn_rate = TURN_RATE_SLOW if speed < 8 else TURN_RATE_FAST
                    mario.face_yaw += diff * turn_rate
            ...
            if cam_mode == 0 and mario.speed > 5:
        ...
        if current_state == STATE_TITLE:
            ...
            # Subtitle
            sub1 = font_subtitle.render("AC'S SM64 PC PORT — PY EDITION 0.2", True, WHITE)
            ...
            # Bottom text
            ver = font_small.render("Castle hub + 15 courses + secrets + Bowser | Star doors like SM64", True, METAL_GREY)
            ...
            ctrl_hint = font_small.render(
                "WASD:Stick  IJKL:C-buttons  SPACE:A  Z/Ctrl:Z  B/X:Dive  Shift:Run  Z+SPACE:LongJump  E/F:Use  R:Cam  TAB/M:Mouse",
                True, (120, 120, 120))
            ...
            pause_items = [
                ...
                ("", WHITE),
                ("WASD:Stick  IJKL:C-buttons  E/F:Use  TAB/M:Mouse", METAL_GREY),
            ]
