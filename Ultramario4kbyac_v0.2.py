...
# Controls: WASD=Move  Q/E=Camera  F=Enter  TAB=MouseCam
# =======================================================================
...
def main():
    ...
    while running:
            ...
            # === IN-GAME ===
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        # Toggle mouse camera (SM64 PC port mouselook)
        ...
        if current_state == STATE_GAME and mario:
            ...
            # === SM64 PC PORT CAMERA CONTROLS ===
            # Q/E = Camera yaw rotation (C-buttons)
            if keys[pygame.K_q]:
                cam_yaw -= CAM_ANGLE_SPEED
            if keys[pygame.K_e]:
                cam_yaw += CAM_ANGLE_SPEED
            # Arrow Up/Down = Camera pitch
            if keys[pygame.K_UP]:
            ...
            if mario.action not in (ACT_STAR_DANCE, ACT_DEATH, ACT_GROUND_POUND_LAND, ACT_DIVE_SLIDE):
                ...
                # WASD primary + Arrow keys secondary (SM64 PC port allows both)
                go_fwd = keys[pygame.K_w] or keys[pygame.K_UP]
                go_back = keys[pygame.K_s] or keys[pygame.K_DOWN]
                go_left = keys[pygame.K_a] or keys[pygame.K_LEFT]
                go_right = keys[pygame.K_d] or keys[pygame.K_RIGHT]

                # But arrow up/down are ONLY movement when NOT held with Q/E
                # (otherwise they conflict with pitch — sm64ex resolves this
                #  by giving arrows to movement only when mouselook is off)
                if not mouse_cam:
                    # When Q/E camera mode: arrows = pitch only (handled above)
                    go_fwd = keys[pygame.K_w]
                    go_back = keys[pygame.K_s]
                    # Arrow left/right can be alt-movement since Q/E handle camera
                    go_left = keys[pygame.K_a] or keys[pygame.K_LEFT]
                    go_right = keys[pygame.K_d] or keys[pygame.K_RIGHT]
                else:
                    # Mouse cam mode: arrow keys become full movement
                    go_fwd = keys[pygame.K_w] or keys[pygame.K_UP]
                    go_back = keys[pygame.K_s] or keys[pygame.K_DOWN]
                    go_left = keys[pygame.K_a] or keys[pygame.K_LEFT]
                    go_right = keys[pygame.K_d] or keys[pygame.K_RIGHT]
        ...
        if current_state == STATE_TITLE:
            ...
            ctrl_hint = font_small.render(
                "WASD:Move  Q/E:Camera  SPACE:Jump  Z:GP  X:Dive  C:Crouch  SHIFT+SPACE:LongJump  F:Enter  TAB:Mouse",
                True, (120, 120, 120))
            ...
            pause_items = [
                ...
                ("", WHITE),
                ("WASD:Move  Q/E:Cam  F:Enter", METAL_GREY),
            ]
